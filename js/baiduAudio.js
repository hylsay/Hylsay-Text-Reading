  ...
  var baiduAudio = {
    ...
    getToken: function () {
      $.ajax({
        ...
        .fail(function () {
          ...
          $(".hylsay-text-r-info").text(
            "** Token换取失败，可能是RestAPI被关闭导致或后台参数配置错误！**"
          );
        });
    },
    makePlayer: function () {
    ...
    getTextArray: function () {
      if (baiduAudio.tokenData.access_token) {
        ...
        newDom
          .find(
            "#baiduAudioPlayer,iframe,[anti],[copy],pre,img,table,.modal,.donation cf," +
              baiduAudio.tokenData.yuedu_pingbitag
          )
          .remove();
        var text = "";
        $(newDom)
